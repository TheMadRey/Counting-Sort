...
fn count_sort(list: Vec<i32>) -> Vec<i32> {
	let args = Args::parse();

	if list.len() < 2 {
		return list;
	} else {
		let mut counting: Vec<i32> = vec![0; args.max as usize];

		for i in list {
			counting[i as usize] += 1;
		}

		print_vec(&counting, 100);

		let mut modified: Vec<i32> = vec![];

		for (i, el) in counting.iter().enumerate() {
			for _ in 0..*el {
				modified.push(i as i32);
			}
		}

		return modified.into_iter().rev().collect()
	}
}
